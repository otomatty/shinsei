...
const useExtensionSettings = (): UseExtensionSettingsHook => {
  ...
  const marketplaceMap = useMemo(() => {
    const map = _.keyBy(marketplaceEntries.value ?? [], (entry) => entry.id);
    return map;
  }, [marketplaceEntries]);

  const groupedMarketplaceEntries = useMemo(() => {
    const entries = marketplaceEntries.value ?? [];
    const grouped = _.groupBy(entries, (entry) => entry.namespace ?? "default");
    return grouped;
  }, [marketplaceEntries]);

  const groupedMarketplaceData = useMemo(() => {
    const data = Object.entries(groupedMarketplaceEntries).map(([namespace, entries]) => ({
      namespace,
      entries: entries
