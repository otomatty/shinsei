...
function WorkspaceContent(props: WorkspaceProps): React.JSX.Element {
  ...
  // Wrap entire workspace with panel state context
  return (
    <PanelStateContextProvider>
      {/* Data source dialog: for data source selection */}
      {/*
       * DataSourceDialog: Data source connection dialog
       * - Load local files (bag/mcap/foxe)
       * - Remote connections (Websocket/ROS)
       * - Sample datasets
       * - Recent file history
       */}
      {dataSourceDialog.open && <DataSourceDialog />}
      {/* Drag & drop listener: file drop support */}
      {/*
       * DocumentDropListener: File drag & drop handler
       * - Direct drop support for bag/mcap/foxe/json files
       * - FileSystemFileHandle API support (Chrome, etc.)
       * - Automatic player selection and loading on drop
       */}
      <DocumentDropListener onDrop={dropHandler} allowedExtensions={allowedDropExtensions} />
      {/* Sync adapters: external sync event processing */}
      {/*
       * SyncAdapters: Multi-instance synchronization
       * - Timeline sync across multiple Lichtblick instances
       * - Playback state sync via broadcast messages
       */}
      <SyncAdapters />
      {/* Key listener: keyboard shortcut handling */}
      {/*
       * KeyListener: Global keyboard shortcuts
       * - '[' / ']': Toggle left/right sidebars
       * - 'Ctrl+O' / 'Cmd+O': Open file
       * - 'Ctrl+Shift+O' / 'Cmd+Shift+O': Select data source
       */}
      <KeyListener global keyDownHandlers={keyDownHandlers} />
      {/* Main container: entire app content */}
      <div className={classes.container} ref={containerRef} tabIndex={0}>
        {/* AppBar: toolbar component */}
        {/*
         * AppBar: Application top toolbar
         * - Layout management (create/save/load)
         * - Add panel functionality
         * - Variables management and extension settings
         * - Window controls (minimize/maximize/close)
         * - Help menu and account settings
         */}
        {appBar}
        {/* Sidebars: sidebar (left/right) rendering */}
        {/*
         * Sidebars: Overall sidebar system management
         * - 3-column layout via MosaicWithoutDragDropContext (leftbar/children/rightbar)
         * - NewSidebar: New UI format tabbed sidebar
         * - Legacy tab navigation (when enableNewTopNav is disabled)
         * - Each sidebar item wrapped with ErrorBoundary
         * - Resizable sidebar width control
         */}
        <Sidebars
          selectedKey=""
          onSelectKey={() => {}}
        ...
        >
          {/* Panel remount: reset panels on player change */}
          {/*
           * RemountOnValueChange: Force remount on player change
           * - Detect playerId changes and fully remount panel group
           * - Cleanup to prevent lingering old player state
           * - Centralized management with PanelLayout inside Stack
           */}
          <RemountOnValueChange value={playerId}>
            <Stack>
              {/* PanelLayout: render panel arrangement */}
              {/*
               * PanelLayout: Panel arrangement management using Mosaic layout system
               * - Split-pane layout via react-mosaic-component
               * - Each panel wrapped in Suspense for lazy loading
               * - EmptyPanelLayout controls display when no panels selected
               * - PanelRemounter controls remount on panel changes
               */}
              <PanelLayout />
            </Stack>
          </RemountOnValueChange>
          <div style={{ flexShrink: 0 }}>
            {/* PlaybackControls: playback control UI */}
            {/*
             * PlaybackControls: Overall data playback controls
             * - Scrubber: Timeline slider and progress display
             * - PlaybackTimeDisplay: Current time display and input
             * - PlaybackSpeedControls: Playback speed dropdown
             * - Play/pause/seek operation buttons
             * - Repeat functionality and event creation
             */}
            <PlaybackControls
              play={play ?? (() => {})}
              pause={pause ?? (() => {})}
      ...
      {/* Dialogs: various dialogs for settings, errors, etc. */}
      {/*
       * WorkspaceDialogs: Workspace-related dialog group
       * - App settings dialog
       * - File selection dialog
       * - Error display dialog
       * - Common UI for confirmation dialogs, etc.
       */}
      <WorkspaceDialogs />
    </PanelStateContextProvider>
  );
